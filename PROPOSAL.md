...
Optimising that ratio is a trap: the same curve says *call more* read as a rate and *never
...
permuted labels 200 times has a median of **0.762** (p = 0.575). Out-of-fold it scores
0.573 ± 0.024; refit forward in time, the only regime production sees, **0.474 — below chance**,
against **0.608** for `ORDER BY sales_contacts_90d DESC`. Fine when shuffled, inverted in time: the brief's
...
**Its importances point the wrong way.** Importance tracks cardinality (rank correlation +0.77), not signal: a pure-noise column in `intent_score`'s slot earns 75% of its importance. Only its *missingness* carries anything (+0.028
...
two with credible signal — `sales_contacts_90d` (odds ratio 2.2 at ≥4) and `intent_known` — both measure
...
owner confirming the contact window precedes the outcome window*, which nothing states — and
...
**`VALUE`'s proposed ledger prices the result in hours and counts only decisions it changed.** On the synthetic transcripts the score is
...
only proof the system works, and not readable yet:
