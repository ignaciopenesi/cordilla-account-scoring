...
| 3 | **Out of sample it is weak; forward in time it is below chance.** OOF 0.576 ± 0.024; forward-chaining 0.474. `ORDER BY sales_contacts_90d DESC` scores 0.608 on the same rows. | | notebook §2.3, §2.6a · `oof_predictions.npy` |
...
- **The proposal's extraction contract**, re-run live on the same GPU against the 20 synthetic transcripts (`extract_intents.py --mode live --force`): **identical to the cached run** — `ready_to_act` 1.00 with 0 missed and 0 false alarms, `next_step_agreed` 1.00, `intent_level` 0.80 exact / 1.00 within one, hot-vs-cold 0.95, mean confidence 0.975; **0 of 20** accounts changed on either routed field. The output file came back byte-identical. Deterministic on this hardware; a few minutes of wall clock, not timed precisely.

## 5 · Reproduce

```bash
python serving/check.sh                      # everything, ~1 min
