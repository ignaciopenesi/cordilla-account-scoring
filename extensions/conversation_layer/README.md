...
is known — the proposed extension in `serving/README.md`. Until then, nothing here runs in the pipeline.
...
> ⚠️ **Everything in `demo_data/` is fabricated.** Twenty transcripts generated by `make_transcripts.py` from twelve template sentences at known intent levels, so the extractor can be measured; `ground_truth.json` holds the level each was generated from; `intents.json` is the cached extraction (`qwen3:14b`); `intents_rerun_gpu_2026-09-11.json` is a live re-run on GPU — the routed fields identical, the six-level grade moved on 7 of 20. It never touches the two CSVs and nothing derived from it is presented as a finding. The method is the deliverable; the numbers are not.
